...
test('hero warning copy is localized and tells Windows users what to expect', () => {
  assert.match(enSource, /windowsWarning: "Unsigned Windows installer\. SmartScreen may show Unknown publisher or block it\. Verify SHA-256 before running\."/);
  assert.match(viSource, /windowsWarning: "Bộ cài Windows chưa ký số\. SmartScreen có thể báo Unknown publisher hoặc chặn bộ cài\. Hãy kiểm tra SHA-256 trước khi chạy\."/);
});
